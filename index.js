...
var gMaps = { infoWindow: null, map: null, region: null, clusterer: null }
...
class Filters {
  ...
  static byTime(sighting) {
    var timeSince = Date.now() - sighting.timestamp
    var range = Filters.timeRange
...
class Database {
  static SPREADSHEET_ID = '1X6HegPWKpgelDp_y7c4qFErmgE1cK9hmNVasQoHzaIE'
  static SHEET_NAME = 'Sheet1'
  static API_KEY = 'AIzaSyDjr0EJNZsRfC5lmZZmjKhILjMTpsFB-Hg'
  ...
  static async fetchFromApi() {
    var response = await fetch(Database.SHEET_URL)
  ...
  static async loadData() {
    var data = await Database.fetchFromApi()
  ...
  static convertRow(row) {
    // get each of the cells in the row and assigning to variables (all strings)
    var [
    ...
    var position = coord(parseFloat(latitude), parseFloat(longitude))
    var datetime = new Date(Date.parse(timestamp))
    var isWalking = Database.parseTrueFalse(walking)
    var isRunning = Database.parseTrueFalse(running)
    var isResting = Database.parseTrueFalse(resting)
    var isChasingSomething = Database.parseTrueFalse(chasingSomething)
    var hasBirdInMouth = Database.parseTrueFalse(birdInMouth)
    var hasSmallMammalInMouth = Database.parseTrueFalse(smallMammalInMouth)
    var hasClippedEar = Database.parseTrueFalse(clippedEar)
  ...
  static async getFilteredSightings() {
    var sightings = await Database.getSightings()
    var filteredSightings = []
    for (var sighting of sightings) {
  ...
  static async getCSVContent() {
    var filteredSightings = await Database.getFilteredSightings()
    var header = Database.data.header

    var fileContent = header.join(',') + '\n'

    for (var sighting of filteredSightings) {
      var row = []
      for (var cell of sighting.originalRow) {
...
function getTableContent(sightings) {
  ...
  for (var sighting of sightings) {
    ...
    var rowContent = "<tr>"
    for (var cell of sighting.cells) {
...
async function renderMap() {
  var infoWindow = gMaps.infoWindow
  var map = gMaps.map
  var region = gMaps.region
  var clusterer = gMaps.clusterer
...
async function download() {
  var fileContent = await Database.getCSVContent()

  // -- trigger a download
  // source: https://stackoverflow.com/questions/45831191/generate-and-download-file-from-js
